...
from ros_sample.srv import SetVelocity
from ros_sample.srv import SetVelocityResponse
...
def velocity_handler(req):
    vel = Twist()
    is_set_success = True
    if req.linear_velocity <= MAX_LINEAR_VEL and (req.linear_velocity >= MIN_LINEAR_VEL):
	vel.linear.x = req.linear_velocity
    else:
	is_set_success = False
    if req.angular_velocity <= MAX_ANGULAR_VEL and (req.angular_velocity >= MIN_ANGULAR_VEL):
